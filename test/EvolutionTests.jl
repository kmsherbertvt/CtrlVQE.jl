module EvolutionTests
    ...
    import CtrlVQE: CWRTDevice, RWRTDevice, TransmonDevice
    ...
    """
    function TestDevice(devicetype::Type{<:DeviceType}, m::Int, Ω, Δ)
        @assert all(Δ .== 0)
        n = length(Ω)
        return devicetype{m}(
            ω[1:n], δ[1:n], g[1:n-1], Quple[Quple(q,q+1) for q in 1:n-1],
            T, [transpose(Ω);;],
        )
    end

    function TestDevice(::Type{TransmonDevice}, m::Int, Ω, Δ)
        n = length(Ω)
        return TransmonDevice{m}(
            ω[1:n], δ[1:n], g[1:n-1], Quple[Quple(q,q+1) for q in 1:n-1],
            1:n,
            [CtrlVQE.Constant(Ω[q]) for q in 1:n],
            [CtrlVQE.Constant(Δ[q])  for q in 1:n],
        )
    end
...
@testset "RWRTDevice" begin
    devicetype = CtrlVQE.RWRTDevice
    @test testinfidelity(devicetype, 2, real.(Ω)[1:1], zero(Δ[1:1])) < 1e-5 # SINGLE QUBIT
    @test testinfidelity(devicetype, 3, real.(Ω)[1:1], zero(Δ[1:1])) < 1e-5# SINGLE QUTRIT
    @test testinfidelity(devicetype, 2, real.(Ω)[1:2], zero(Δ[1:2])) < 1e-5 # TWO QUBITS
    @test testinfidelity(devicetype, 3, real.(Ω)[1:2], zero(Δ[1:2])) < 1e-5 # TWO QUTRITS
end

@testset "CWRTDevice" begin
    devicetype = CtrlVQE.CWRTDevice
    @test testinfidelity(devicetype, 2, Ω[1:1], zero(Δ[1:1])) < 1e-5    # SINGLE QUBIT
    @test testinfidelity(devicetype, 3, Ω[1:1], zero(Δ[1:1])) < 1e-5    # SINGLE QUTRIT
    @test testinfidelity(devicetype, 2, Ω[1:2], zero(Δ[1:2])) < 1e-5    # TWO QUBITS
    @test testinfidelity(devicetype, 3, Ω[1:2], zero(Δ[1:2])) < 1e-5    # TWO QUTRITS
end
