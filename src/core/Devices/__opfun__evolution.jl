...
function evolver(
    ...
    m = nlevels(device)
    n = nqubits(device)
    ū = @temparray(Complex{eltype(device)}, (m,m,n), :uncoupledevolver)
...
function evolve!(
    ...
    N = nstates(device)
    U = @temparray(Complex{eltype(device)}, (N,N), :staticevolver)
...
function evolve!(
    ...
    m = nlevels(device)
    n = nqubits(device)
    ops = @temparray(Complex{eltype(device)}, (m,m,n), :qubitevolve)
    for p in 1:n
        if p == op.q
